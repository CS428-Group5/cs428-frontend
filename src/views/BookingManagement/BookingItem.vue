<template>
  <div class="border border-solid border-gray-light rounded-2xl p-4 mb-6">
    <p class="font-bold text-lg mb-2">
      ...
      {{
        isMentor
          ? session.mentor_first_name
          : session.mentee_first_name + ' ' + isMentor
          ? session.mentor_last_name
    ...
    <div v-if="session.cancelled_by == 0">
      <a :href="session.event_link">
        <button
          class="w-fit h-12 mr-4 px-4 py-3 text-white rounded-xl"
          style="background-color: #599bff"
          @click="() => goToMeeting(session)"
        >
          Meet
        </button>
      </a>
      <button
        class="w-fit h-12 px-4 py-3 text-red rounded-xl"
    ...
    <div
      v-else-if="
        (session.cancelled_by == 1 && isMentor) || (session.cancelled_by == 2 && !isMentor)
      "
    >
...
export default {
  ...
  methods: {
    ...
    async goToMeeting(session) {
      await client.get(`/session/booked_session/${session.id}/meeting_link`).then((res) => {
        const meeting_url = res.data.meeting_url

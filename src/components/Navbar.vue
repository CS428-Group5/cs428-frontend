<template>
  <div class="grid grid-cols-12 gap-x-6 pt-3 pb-8">
    ...
    <input type="text" placeholder="Search for a mentor" name="from" id="from"
    ...
    <div v-if="!isLogin" class="col-span-3 flex justify-end gap-6">
      <button type="button" class="bg-white w-fit px-4 py-3 text-blue rounded-2xl" @click="this.$router.push(`/sign-up`)">Sign up</button>
      <button type="button" class="bg-blue w-fit px-4 py-3 text-white rounded-2xl" @click="this.$router.push(`/login`)">Log in</button>
    </div>
    <div v-else class="col-span-3 flex justify-end items-center gap-6 relative">
      ...
      <img @click="toggleDropDown" class="h-11 w-11 aspect-square rounded-3xl" src="@/assets/avatar.jpg" alt="avatar" />
      <div v-show="showDropDown" class="z-10 absolute top-14 right-0 bg-white divide-y divide-gray-100 rounded-lg shadow">
        <ul v-if="!isMentor">
